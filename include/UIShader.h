...
class UIShader {
  ...
  public:
    ...
    void setModelMatrix(const float *matrix) { glUniformMatrix4fv(modelLoc, 1, GL_FALSE, matrix); }

    void setViewMatrix(const float *matrix) { glUniformMatrix4fv(viewLoc, 1, GL_FALSE, matrix); }

    void setProjectionMatrix(const float *matrix) { glUniformMatrix4fv(projLoc, 1, GL_FALSE, matrix); }
    ...
    void drawCrosshair(int screenWidth, int screenHeight, int size = 10, float r = 1.0f, float g = 1.0f,
                       float b = 1.0f);
    void drawHotbar(int windowWidth, int windowHeight, int hotbarNumSelected, vector<int> blockTypes);
    void drawCreativeInventory(int windowWidth, int windowHeight, vector<int> itemsInInventory, int tabTopSelected,
                               vector<int> blocksInHotbar);
