    ...
    for (int x = 0; x < 16; x++) {
        for (int z = 0; z < 16; z++) {
            ...
            for (int y = 0; y <= continentalHeight; y++) {
                ...
                chunk->setBlockSafe(x, y, z, block);
            }
            ...
            if (continentalHeight < 60) {
                for (int y = continentalHeight + 1; y <= 60; y++) {
                    ...
                    chunk->setBlockSafe(x, y, z, waterBlock);
                }
            }
        }
    }
        ...
        for (int z = 0; z < 16; z++) {
            ...
            if (random < 0.002f) {                  // 0.2% probabilidad
                ...
                for (int y = 0; y < trunkHeight; y++) {
                    ...
                    wood.active = true;
                    wood.type = 21;
                    chunk->setBlockSafe(x, groundY + y + 1, z, wood);
                }
                    ...
                    for (int dx = -currentRadius; dx <= currentRadius; dx++) {
                        for (int dz = -currentRadius; dz <= currentRadius; dz++) {
                            ...
                            if (localX < 0 || localX >= 16 || localZ < 0 || localZ >= 16)
                                continue;
                            chunk->setBlockSafe(localX, y, localZ, leaf);
                        }
                    }
                }
            }
                ...
                for (auto [nx, nz] : vecinos) {
                    ...
                    if (chunk) { // existe en el mapa
                        bool yaEnCola = false;
                        {
                            ...
                            yaEnCola = (requestedUpdateChunks.find({nx, nz}) != requestedUpdateChunks.end());
                        }
                        if (!yaEnCola) {
                            chunk->setUpdate();
                        }
        ...
        if (nivel == 0) {
            ...
            if (chunk) {
                ...
                if (chunk->needsUpdate) {
                    ...
                    if (requestedUpdateChunks.find({chunkX, chunkZ}) == requestedUpdateChunks.end()) {
                        {
...
void World::startCreationThread() {
    ...
    // meshThread1 = thread(&World::loopCreation, this);
    // meshThread2 = thread(&World::loopCreation, this);
}
