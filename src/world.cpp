...
void World::generateFlatWorld(int width, int depth) {
  ...
  for (int cx = 0; cx < chunksInX; cx++) {
    for (int cz = 0; cz < chunksInZ; cz++) {
      chunks[cx][cz] = make_unique<Chunk>();
...
void World::setBlockSafe(int x, int y, int z, Block block) {
  ...
  if (x - posChunk.x * 16 == 15) {
    Chunk *chunkXPlus = getChunk(posChunk.x + 1, posChunk.y);
    if (chunkXPlus) {
      chunkXPlus->setUpdate();
    }
  } else if (x - posChunk.x * 16 == 0) {

    Chunk *chunkXminus = getChunk(posChunk.x - 1, posChunk.y);
    if (chunkXminus) {
      chunkXminus->setUpdate();
    }
  }
  if (z - posChunk.y * 16 == 15) {

    Chunk *chunkZPlus = getChunk(posChunk.x, posChunk.y + 1);
    if (chunkZPlus) {
      chunkZPlus->setUpdate();
    }
  } else if (z - posChunk.y * 16 == 0) {
    Chunk *chunkZMinus = getChunk(posChunk.x, posChunk.y - 1);
    if (chunkZMinus) {
      chunkZMinus->setUpdate();
    }
  }
}
void World::createChunk(int cx, int cz) {
  ...
  for (int x = 0; x < 16; x++) {
    for (int z = 0; z < 16; z++) {
      ...
      uint32_t hash = worldX * 374761393u + worldZ * 668265263u;
      hash = (hash ^ (hash >> 13)) * 1274126177u;
      if (chunk->getBlock(x, groundY - 1, z).type != 4)
        continue;
      float random = (hash & 0xFFFFFF) / float(0xFFFFFF);
...
void World::render(vec3 cameraPos, mat4 view, mat4 projection, mat4 renderView,
  ...
  {
    lock_guard<mutex> lockResult(mutexChunkResult);
    ...
    while (!chunkResultQueue.empty()) {
          ...
          {cx, cz}, {cx + 1, cz}, {cx - 1, cz}, {cx, cz + 1}, {cx, cz - 1}};
      {
        for (auto [nx, nz] : vecinos) {
          ...
          if (chunk) { // existe en el mapa
            bool yaEnCola = false;
            {
              lock_guard<mutex> lock(setUpdateChunk);
              yaEnCola = (requestedUpdateChunks.find({nx, nz}) !=
                          requestedUpdateChunks.end());
            }
            if (!yaEnCola) {
              chunk->setUpdate();
            }
  ...
  for (int nivel = 0; nivel <= generateDist; nivel++) {
    if (nivel == 0) {
      ...
      if (chunk) {
        chunk->render();
        unique_lock<mutex> lock(chunk->mutexUpdate);
        if (chunk->needsUpdate) {
          lock.unlock();
          unique_lock<mutex> lockSet(setUpdateChunk);
          if (requestedUpdateChunks.find({chunkX, chunkZ}) ==
              requestedUpdateChunks.end()) {
            {
              lock_guard<mutex> lock(mutexChunkUpdateRequest);
              chunkRequestUpdateQueue.push({chunkX, chunkZ});
            }
            cout << "Agrego chunk " << chunkX << " " << chunkZ << endl;
            requestedUpdateChunks.insert({chunkX, chunkZ});
          }
          meshCV.notify_one();
        }
        ...
        if (requestedChunks.find({chunkX, chunkZ}) == requestedChunks.end()) {
          size_t requestQueueSize;
          {
            ...
            requestQueueSize = chunkRequestQueue.size();
          }
          if (requestQueueSize < maxChunksToGenerate) {
            {
              lock_guard<mutex> lock(mutexChunkRequest);
        ...
        if (isBoxVisible(planes, min, max)) {
          ...
          if (chunk) {
            if (nivel <= renderDist) {
              ...
              if (chunk->needsUpdate) {
                lock.unlock();
                lock_guard<mutex> lockSet(setUpdateChunk);
                if (requestedUpdateChunks.find({chunkX, chunkZ}) ==
                    requestedUpdateChunks.end()) {
                  {
                    lock_guard<mutex> lock(mutexChunkUpdateRequest);
...
void World::loopCreation() {
  while (threadRunning) {
    ...
    RequestCV.wait(
        lock, [this] { return !chunkRequestQueue.empty() || !threadRunning; });
    if (!threadRunning)
      break;
    while (!chunkRequestQueue.empty()) {
      ...
      lock.unlock(); // Libero mutexChunkRequest
      createChunk(x, z);
      lock.lock();
    }
  }
...
void World::loopMesh() {
  while (threadRunning) {
    ...
    if (!threadRunning)
      break;
    while (!chunkRequestUpdateQueue.empty()) {
      ...
      if (chunk) {
        ...
        {
          lock_guard<mutex> lock(setUpdateChunk);
          requestedUpdateChunks.erase({x, z});
        }
      }
      lock.lock();
    }
