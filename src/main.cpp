...
bool detectBlock(Screen& screen, World& mundo, vec3& blockSelect, vec3& blockFace, bool& colocar) {
    ...
    while (!encontreBloque && x < 4) {
        ...
        if (mundo.getBlock(posibleBloque.x, posibleBloque.y, posibleBloque.z).active) {
    ...
    while (screen.isRunning()) {
        ...
        if (detectBlock(screen, world, blockPos, blockFace, colocar)) {
            ...
            if (screen.wasRightClicked() && colocar) {
                ...
                chunk->setBlock(blockFace.x - offsetx, blockFace.y, blockFace.z - offsetz, block);
            }
            if (screen.wasLeftClicked()) {
                ...
                int offsetx = chunk->getNroChunkX() * 16;
                int offsetz = chunk->getNroChunkZ() * 16;
                chunk->setBlock(blockPos.x - offsetx, blockPos.y, blockPos.z - offsetz, block);
            }
        }
