  ...
  for (int i = 0; i < FILAS; i++) {
    for (int j = 0; j < COLUMNAS; j++) {
      if (tipos[i][j] != 0 && !procesado[i][j]) {
        ...
        while (i + ancho < FILAS && tipos[i + ancho][j] == tipo_actual &&
               !procesado[i + ancho][j]) {
          ancho++;
        }
...
void Chunk::generateMesh() {
  ...
  for (int x = 0; x < 16; x++) {
    ...
    for (const Rectangulo &r : rectsDer) {
      cargarVertices(r, 0, 1, x, r.tipoBloque, newVertexData, newIndexData,
                     newVertexCount, newCaras);
    }
  ...
  for (int y = 0; y < 256; y++) {
    ...
    for (const Rectangulo &r : rectsInf) {
      cargarVertices(r, 1, -1, y, r.tipoBloque, newVertexData, newIndexData,
                     newVertexCount, newCaras);
    }
  ...
  for (int z = 0; z < 16; z++) {
    ...
    for (const Rectangulo &r : rectsFrontal) {
      cargarVertices(r, 2, 1, z, r.tipoBloque, newVertexData, newIndexData,
                     newVertexCount, newCaras);
    }
...
Block Chunk::getBlock(int x, int y, int z) const { return blocks[x][y][z]; }
