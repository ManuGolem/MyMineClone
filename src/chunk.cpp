...
void Chunk::cargarVertices(const Rectangulo& r, int eje, int direccion, int fijo, int tipo_bloque, vector<float>& vData, vector<unsigned int>& iData, unsigned int& vCount, int& carasLocal) {
    float base = vCount;
...
void Chunk::generateMesh() {
    ...
    for (int x = 0; x < 16; x++) {
        ...
        for (const Rectangulo& r : rectsDer) {
            cargarVertices(r, 0, 1, x, r.tipoBloque, newVertexData, newIndexData, newVertexCount, newCaras);
        }
    ...
    for (int y = 0; y < 256; y++) {
        ...
        for (const Rectangulo& r : rectsInf) {
            cargarVertices(r, 1, -1, y, r.tipoBloque, newVertexData, newIndexData, newVertexCount, newCaras);
        }
    ...
    for (int z = 0; z < 16; z++) {
        ...
        for (const Rectangulo& r : rectsFrontal) {
            cargarVertices(r, 2, 1, z, r.tipoBloque, newVertexData, newIndexData, newVertexCount, newCaras);
        }
...
void Chunk::setBlockSafe(int x, int y, int z, const Block& block) {
    blocks[x][y][z] = block;
    {
        lock_guard<mutex> lock(mutexUpdate);
        needsUpdate = true;
...
void Chunk::setBlock(int x, int y, int z, const Block& block) {
    ...
    if (x == 15) {
        lock_guard<mutex> lock(world->mapChunks);
...
void Chunk::render() {
    if (!chunkBuffer) {
        chunkBuffer = make_unique<ChunkBuffer>(); // solo en main thread
    }
